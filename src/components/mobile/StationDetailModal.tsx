...
import { brandColors, MOCK_USER_LOCATION } from "@/lib/mock-data";
...
export default function StationDetailModal({ station, open, onClose }: StationDetailModalProps) {
    ...
    if (navigator.share) {
      try {
        await navigator.share({
          title: station.name,
          text: `${station.name} — Unleaded P${station.prices.unleaded.toFixed(2)}/L`,
            ...
            <div className="px-5 pb-8">
              ...
              >
                <div className="min-w-0 flex-1">
                  ...
                  <p className="text-white/80 text-sm truncate">{station.address}</p>
                </div>
                <button
                  type="button"
                  onClick={onClose}
                  className="flex-shrink-0 ml-3 text-white/80 hover:text-white p-1"
                  aria-label="Isara"
                >
              ...
              <div className="bg-gray-50 rounded-2xl p-4 mb-4">
                ...
                <div className="grid grid-cols-2 gap-2">
                  {(Object.keys(fuelTypeLabels) as Array<keyof typeof fuelTypeLabels>).map((key) => {
                    const price = station.prices[key as keyof typeof station.prices];
                    return (
                      <div
                        key={key}
                        className="flex items-center justify-between bg-white rounded-xl px-3 py-2.5"
                      >
                        <span className="text-xs text-gray-500">{fuelTypeLabels[key]}</span>
                        <span className="text-sm font-bold text-gray-900">
                          {typeof price === "number" ? `P${price.toFixed(2)}` : "---"}
                        </span>
                      </div>
                    );
                  })}
                ...
                <div className="mb-5">
                  ...
                  <div className="flex flex-wrap gap-2">
                    {station.amenities.map((a) => (
                      <span
                        key={a}
                        className="inline-flex items-center gap-1.5 px-3 py-1.5 bg-gray-100 rounded-full text-xs text-gray-700 font-medium"
                      >
              ...
              <div className="flex gap-2">
                <a
                  ...
                  className="flex-1 flex items-center justify-center gap-2 px-4 py-3.5 bg-orange-500 text-white font-bold rounded-xl hover:bg-orange-600 transition-colors text-sm"
                >
                ...
                <a
                  ...
                  target="_blank"
                  rel="noopener noreferrer"
                  className="flex-1 flex items-center justify-center gap-2 px-4 py-3.5 bg-blue-500 text-white font-bold rounded-xl hover:bg-blue-600 transition-colors text-sm"
                >
                  <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" strokeWidth={2} stroke="currentColor">
                ...
                <button
                  type="button"
                  onClick={handleShare}
                  className="flex items-center justify-center px-4 py-3.5 bg-gray-100 text-gray-700 font-bold rounded-xl hover:bg-gray-200 transition-colors text-sm"
                  aria-label="I-share"
                >
